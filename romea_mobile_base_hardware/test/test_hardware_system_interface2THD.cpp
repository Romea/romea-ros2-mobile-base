...
class TestHardwareSystemInterface2THD : public ::testing::Test
{
protected:
  void SetUp() override
  {
    hardware_infos = parse_hardware_info("test_hardware_system_interface2THD.xacro");
...
TEST_F(
  TestHardwareSystemInterface2THD,
  checkFactoryCreatesInterfaceFromPrefixedParameters)
{
  auto hardware_interface =
    romea::ros2::make_hardware_interface(hardware_infos[0], "2THD", "mobile_base");

  auto & interface = dynamic_cast<romea::ros2::HardwareInterface2THD &>(*hardware_interface);
  ...
  checkSystemInterfaceExportsStateAndCommandInterfaces)
{
  ...
  expect_interface_name(state_interfaces[6], "front_left_idler_wheel_spinning_joint/position");
  expect_interface_name(state_interfaces[9], "front_right_idler_wheel_spinning_joint/position");
  expect_interface_name(state_interfaces[12], "rear_left_idler_wheel_spinning_joint/position");
  expect_interface_name(state_interfaces[15], "rear_right_idler_wheel_spinning_joint/position");
