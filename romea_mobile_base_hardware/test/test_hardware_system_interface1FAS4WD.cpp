...
class TestHardwareSystemInterface1FAS4WD : public ::testing::Test
{
protected:
  void SetUp() override
  {
    hardware_infos = parse_hardware_info("test_hardware_system_interface1FAS4WD.xacro");
...
TEST_F(
  TestHardwareSystemInterface1FAS4WD,
  checkFactoryCreatesInterfaceFromPrefixedParameters)
{
  auto hardware_interface =
    romea::ros2::make_hardware_interface(hardware_infos[0], "1FAS4WD", "mobile_base");

  auto & interface = dynamic_cast<romea::ros2::HardwareInterface1FAS4WD &>(*hardware_interface);

  auto joint_names = interface.get_joint_names();
  ASSERT_EQ(joint_names.size(), 5u);
  ...
  EXPECT_EQ(joint_names[3], "rear_left_wheel_spinning_joint");
  EXPECT_EQ(joint_names[4], "rear_right_wheel_spinning_joint");
}
  ...
  checkSystemInterfaceLoadsDynamicInterface)
{
  ...
  auto & interface =
    system_interface.hardware_interface<romea::ros2::HardwareInterface1FAS4WD>("mobile_base");
  ...
  checkSystemInterfaceExportsStateAndCommandInterfaces)
{
  ...
  expect_interface_name(state_interfaces[7], "rear_left_wheel_spinning_joint/position");
  expect_interface_name(state_interfaces[10], "rear_right_wheel_spinning_joint/position");
  expect_interface_name(state_interfaces[13], "front_left_wheel_steering_joint/position");
  expect_interface_name(state_interfaces[14], "front_right_wheel_steering_joint/position");

  auto command_interfaces = system_interface.export_command_interfaces();
  ASSERT_EQ(command_interfaces.size(), 5u);
  ...
  expect_interface_name(command_interfaces[3], "rear_left_wheel_spinning_joint/velocity");
  expect_interface_name(command_interfaces[4], "rear_right_wheel_spinning_joint/velocity");
}
  ...
  checkCommandValuesArePackedIntoJointState)
{
  ...
  ASSERT_EQ(command.name.size(), 5u);
  ...
  EXPECT_EQ(command.name[3], "rear_left_wheel_spinning_joint");
  EXPECT_EQ(command.name[4], "rear_right_wheel_spinning_joint");
  ...
  checkJointStateFeedbackValuesAreExportedIntoStateInterfaces)
{
  ...
  auto feedback = romea::ros2::make_joint_state_msg(5);
  ...
  feedback.name[3] = "rear_left_wheel_spinning_joint";
  feedback.name[4] = "rear_right_wheel_spinning_joint";
  ...
  feedback.position[3] = 8.0;
  feedback.velocity[3] = 9.0;
  feedback.effort[3] = 10.0;
  feedback.position[4] = 11.0;
  feedback.velocity[4] = 12.0;
  feedback.effort[4] = 13.0;
  ...
  checkCommandValuesAreAvailableAsCoreHardwareCommand)
{
  ...
  EXPECT_DOUBLE_EQ(command.rearLeftWheelSpinningSetPoint, 4.0);
  EXPECT_DOUBLE_EQ(command.rearRightWheelSpinningSetPoint, 5.0);
}
  ...
  checkCoreHardwareStateValuesAreExportedIntoStateInterfaces)
{
  ...
  romea::core::HardwareState1FAS4WD state;
  ...
  state.rearLeftWheelSpinningMotion.position = 8.0;
  state.rearLeftWheelSpinningMotion.velocity = 9.0;
  state.rearLeftWheelSpinningMotion.torque = 10.0;
  state.rearRightWheelSpinningMotion.position = 11.0;
  state.rearRightWheelSpinningMotion.velocity = 12.0;
  state.rearRightWheelSpinningMotion.torque = 13.0;
