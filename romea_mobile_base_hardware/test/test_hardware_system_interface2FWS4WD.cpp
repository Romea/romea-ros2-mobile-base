...
class TestHardwareSystemInterface2FWS4WD : public ::testing::Test
{
protected:
  void SetUp() override
  {
    hardware_infos = parse_hardware_info("test_hardware_system_interface2FWS4WD.xacro");
...
TEST_F(
  TestHardwareSystemInterface2FWS4WD,
  checkFactoryCreatesInterfaceFromPrefixedParameters)
{
  auto hardware_interface =
    romea::ros2::make_hardware_interface(hardware_infos[0], "2FWS4WD", "mobile_base");

  auto & interface = dynamic_cast<romea::ros2::HardwareInterface2FWS4WD &>(*hardware_interface);
  ...
  checkSystemInterfaceLoadsDynamicInterface)
{
  ...
  auto & interface =
    system_interface.hardware_interface<romea::ros2::HardwareInterface2FWS4WD>("mobile_base");
  ...
  checkCommandValuesArePackedIntoJointState)
{
  ...
  EXPECT_EQ(command.name[1], "front_right_wheel_steering_joint");
  ...
  checkCoreHardwareStateValuesAreExportedIntoStateInterfaces)
{
  ...
  romea::core::HardwareState2FWS4WD state;
