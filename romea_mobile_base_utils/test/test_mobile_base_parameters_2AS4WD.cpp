...
class TestMobileBaseParams2AS4WD : public ::testing::Test
{
protected:
  ...
  void SetUp() override
  {
    ...
    no.arguments({"--ros-args","--params-file",std::string(TEST_DIR)+"/test_mobile_base_parameters_2AS4WD.yaml"});
    node = std::make_shared<rclcpp::Node>("test_mobile_base_parameters_2AS4WD", no);
  }
...
TEST_F(TestMobileBaseParams2AS4WD, checkGetInfo)
{
  romea::declare_mobile_base_info_2AS4WD(node,"base");
  auto base_info =romea::get_mobile_base_info_2AS4WD(node,"base");
  ...
  EXPECT_DOUBLE_EQ(base_info.axlesSteeringControl.command.maximalAngle,200);
  EXPECT_DOUBLE_EQ(base_info.axlesSteeringControl.command.maximalAngularSpeed,201);
  EXPECT_DOUBLE_EQ(base_info.axlesSteeringControl.sensor.angleStd,202);
  EXPECT_DOUBLE_EQ(base_info.axlesSteeringControl.sensor.angleRange,203);
...
//TEST_F(TestMobileBaseParams2AS4WD, checkGetJointMappings)
//{
//  romea::declare_joint_mappings_2AS4WD(node,"base.joints");
//  auto base_info =romea::get_joint_mappings_2AS4WD(node,"base.joints");
