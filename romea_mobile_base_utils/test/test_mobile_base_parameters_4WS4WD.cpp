...
//romea
#include "romea_mobile_base_utils/params/mobile_base_parameters4WS4WD.hpp"


class TestMobileBaseParams4WS4WD : public ::testing::Test
{
protected:
  ...
  void SetUp() override
  {
    ...
    no.arguments({"--ros-args","--params-file",std::string(TEST_DIR)+"/test_mobile_base_parameters_4WS4WD.yaml"});
    node = std::make_shared<rclcpp::Node>("test_mobile_base_parameters_4WS4WD", no);
  }
...
{
  romea::MobileBaseInfo4WS4WD base_info;
  romea::declare_mobile_base_info_4WS4WD(node,"base");
  romea::get_mobile_base_info_4WS4WD(node,"base",base_info);
  ...
  EXPECT_DOUBLE_EQ(base_info.wheelsSteeringControl.command.maximalAngle,200);
  EXPECT_DOUBLE_EQ(base_info.wheelsSteeringControl.command.maximalAngularSpeed,201);
  EXPECT_DOUBLE_EQ(base_info.wheelsSteeringControl.sensor.angleStd,202);
  EXPECT_DOUBLE_EQ(base_info.wheelsSteeringControl.sensor.angleRange,203);
...
TEST_F(TestMobileBaseParams4WS4WD, checkGetJointMappings)
{
  romea::declare_joint_mappings_4WS4WD(node,"base.joints");
  auto base_info =romea::get_joint_mappings_4WS4WD(node,"base.joints");
}
