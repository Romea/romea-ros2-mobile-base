...
void declare_mobile_base_info_2AS4WD(std::shared_ptr<Node> node, const std::string & parameters_ns)
{
  declare_two_wheeled_axles_info(node, full_param_name(parameters_ns, "geometry"));

  declare_steering_angle_control_info(
    node, full_param_name(parameters_ns, "axles_steering_control"));
...
template<typename Node>
core::MobileBaseInfo2AS4WD get_mobile_base_info_2AS4WD(
  std::shared_ptr<Node> node, const std::string & parameters_ns)
{
  return {
    ...
    get_steering_angle_control_info(node, full_param_name(parameters_ns, "axles_steering_control")),
