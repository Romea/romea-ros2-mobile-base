  ...
  std::shared_ptr<Node> node, const std::string & parameters_ns)
{
  ...
  declare_parameter_with_default<double>(
    node, parameters_ns, "sensor.angle_range", std::numeric_limits<double>::max());
...
{
  return {
    {get_parameter<double>(node, parameters_ns, "sensor.angle_std"),
...
template<typename Node>
void declare_wheel_speed_control_info(std::shared_ptr<Node> node, const std::string & parameters_ns)
{
  declare_parameter<double>(node, parameters_ns, "sensor.speed_std");

  declare_parameter_with_default<double>(
    node, parameters_ns, "sensor.speed_range", std::numeric_limits<double>::max());
...
{
  return {
    {get_parameter<double>(node, parameters_ns, "sensor.speed_std"),
