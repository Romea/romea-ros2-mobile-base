#ifndef _romea_MobileBaseParameters1FWS2RWD_hpp_
#define _romea_MobileBaseParametersFWS2RWD_hpp_

#include <rclcpp/node.hpp>
#include <romea_core_mobile_base/info/MobileBaseInfo1FWS2RWD.hpp>

namespace romea {


void declare_mobile_base_info(std::shared_ptr<rclcpp::Node> node,
                              const std::string & parameters_ns,
                              const MobileBaseInfo1FWS2RWD & mobile_base_info = MobileBaseInfo1FWS2RWD());

void get_mobile_base_info(std::shared_ptr<rclcpp::Node> node,
                          const std::string & parameters_ns,
                          MobileBaseInfo1FWS2RWD & mobile_base_info);

}

#endif
