...
void declare_mobile_base_info_2WD(std::shared_ptr<Node> node, const std::string & parameters_ns)
{
  declare_wheeled_axle_info(node, full_param_name(parameters_ns, "geometry"));
...
template<typename Node>
core::MobileBaseInfo2WD get_mobile_base_info_2WD(
  std::shared_ptr<Node> node, const std::string & parameters_ns)
{
  return {
    get_wheeled_axle_info(node, full_param_name(parameters_ns, "geometry")),
