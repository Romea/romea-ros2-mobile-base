...
def wait_for_controller_manager(node, controller_manager, timeout_duration):
    ...
    print("controller_manager", controller_manager)
    ...
    try:
        ...
        if is_controller_loaded(node, controller_manager_name, prefixed_controller_name):
            ...
            if controller_type:
                ...
                if result.successful:
                    node.get_logger().info(
                        bcolors.OKCYAN + 'Set controller type to "' + controller_type + '" for ' + bcolors.BOLD +
                        prefixed_controller_name + bcolors.ENDC)
                else:
                    node.get_logger().fatal(bcolors.FAIL + 'Could not set controller type to "' + controller_type +
                                            '" for ' + bcolors.BOLD + prefixed_controller_name + bcolors.ENDC)
                    return 1
        ...
        if param_file:
            load_parameter_file(node=node, node_name=prefixed_controller_name, parameter_file=param_file,
                                use_wildcard=True)
            node.get_logger().info(
                bcolors.OKCYAN + 'Loaded parameters file "' + param_file + '" for ' + bcolors.BOLD +
                prefixed_controller_name + bcolors.ENDC)
