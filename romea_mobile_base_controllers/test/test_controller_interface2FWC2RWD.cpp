...
class TestControllerInterface2FWC2RWD : public ::testing::Test
{
protected:
  ...
  void SetUp() override
  {
    ...
    no.arguments(
      {"--ros-args",
       "-p",
       "joints.rear_left_wheel_spinning_joint_name:=J1",
       "-p",
       "joints.rear_right_wheel_spinning_joint_name:=J2"});

    node = std::make_shared<romea::ros2::HardwareInterfaceNode>(
      "test_interface_controller_2FWC2RWD", no);

    state_values.resize(2);
    command_values.resize(2);

    romea::ros2::ControllerInterface2FWC2RWD::declare_joints_names(node, "joints");
    joints_names = romea::ros2::ControllerInterface2FWC2RWD::get_joints_names(node, "joints");
    ...
    controller_interface = std::make_unique<romea::ros2::ControllerInterface2FWC2RWD>(mobile_info);
  }
  ...
  romea::core::MobileBaseInfo2FWC2RWD mobile_info;
  std::vector<std::string> joints_names;
  std::unique_ptr<romea::ros2::ControllerInterface2FWC2RWD> controller_interface;
};

TEST_F(TestControllerInterface2FWC2RWD, checkHardwareInterfaceNames)
{
  auto hardware_interface_names =
    romea::ros2::ControllerInterface2FWC2RWD::hardware_interface_names(joints_names);
  EXPECT_STREQ(hardware_interface_names[0].c_str(), "J1/velocity");
  EXPECT_STREQ(hardware_interface_names[1].c_str(), "J2/velocity");
}

TEST_F(TestControllerInterface2FWC2RWD, checkWrite)
{
  romea::core::OdometryFrame2WD command;
  command.leftWheelLinearSpeed = 3;
  command.rightWheelLinearSpeed = 4;

  controller_interface->write(command, command_loaned_interfaces);
  EXPECT_EQ(command_values[0], 6);
  EXPECT_EQ(command_values[1], 8);
}

TEST_F(TestControllerInterface2FWC2RWD, checkRead)
{
  state_values[0] = 6;
  state_values[1] = 8;
  ...
  EXPECT_EQ(measure.leftWheelLinearSpeed, 3);
  EXPECT_EQ(measure.rightWheelLinearSpeed, 4);
}
