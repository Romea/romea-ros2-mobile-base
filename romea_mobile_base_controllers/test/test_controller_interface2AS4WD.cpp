...
protected:
  ...
  void SetUp() override
  {
    ...
    no.arguments(
      {"--ros-args",
       "-p",
       "joints.front_axle_steering_joint_name:=J1",
       "-p",
       "joints.rear_axle_steering_joint_name:=J2",
       "-p",
    ...
    node =
      std::make_shared<romea::ros2::HardwareInterfaceNode>("test_interface_controller_2AS4WD", no);
...
TEST_F(TestControllerInterface2AS4WD, checkHardwareInterfaceNames)
{
  auto hardware_interface_names =
    romea::ros2::ControllerInterface2AS4WD::hardware_interface_names(joints_names);
