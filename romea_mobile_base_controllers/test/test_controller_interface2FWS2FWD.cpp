...
protected:
  ...
  void SetUp() override
  {
    ...
    node = std::make_shared<romea::ros2::HardwareInterfaceNode>(
      "test_interface_controller_2FWS2FWD", no);

    state_values.resize(4);
    command_values.resize(4);

    romea::ros2::ControllerInterface2FWS2FWD::declare_joints_names(node, "joints");
    joints_names = romea::ros2::ControllerInterface2FWS2FWD::get_joints_names(node, "joints");
    ...
    controller_interface = std::make_unique<romea::ros2::ControllerInterface2FWS2FWD>(mobile_info);
  }
  ...
  romea::core::MobileBaseInfo2FWS2FWD mobile_info;
  std::vector<std::string> joints_names;
  std::unique_ptr<romea::ros2::ControllerInterface2FWS2FWD> controller_interface;
};

TEST_F(TestControllerInterface2FWS2FWD, checkHardwareInterfaceNames)
{
  auto hardware_interface_names =
    romea::ros2::ControllerInterface2FWS2FWD::hardware_interface_names(joints_names);
...
{
  romea::core::OdometryFrame2FWS2FWD command;
...
TEST_F(TestControllerInterface2FWS2FWD, checkRead)
{
  ...
  romea::core::OdometryFrame2FWS2FWD measure;
