...
CallbackReturn MobileBaseController<InterfaceType,KinematicType>::on_init()
{
  ...
  try
  {
    declare_joints_mapping_();
    declare_mobile_base_info_();
    declare_command_constraints_();
...
MobileBaseController<InterfaceType,KinematicType>::command_interface_configuration() const
{
  return {controller_interface::interface_configuration_type::INDIVIDUAL,
        controller_interface_->getCommandInterfaceNames()};
}
...
{
  return {controller_interface::interface_configuration_type::INDIVIDUAL,
        controller_interface_->getStateInterfaceNames()};
}
...
{
  odometry_frame_ = controller_interface_->getOdometryFrame();
  //    RCLCPP_INFO_STREAM(node_->get_logger(),"odometry frame measured");
  //    RCLCPP_INFO_STREAM(node_->get_logger(),"\n"<<odometry_frame_);
...
void MobileBaseController<OdometryFrameType,KinematicType>::send_current_command_()
{
  ...
  controller_interface_->setCommand(odometry_frame_);
}
...
{
  controller_interface_->setCommand(OdometryFrame());
}
...
//-----------------------------------------------------------------------------
template <typename InterfaceType, typename KinematicType>
void MobileBaseController<InterfaceType,KinematicType>::declare_joints_mapping_()
{
  declare_parameter_with_default<std::string>(node_,JOINTS_PREFIX_PARAM_NAME,"");
  InterfaceType::declare_joints_mapping(node_,JOINTS_MAPPING_PARAM_NAME);
}

//-----------------------------------------------------------------------------
template <typename InterfaceType, typename KinematicType>
std::map<int,std::string> MobileBaseController<InterfaceType,KinematicType>::load_joints_mapping_()
{
  std::string prefix = get_parameter<std::string>(node_,JOINTS_PREFIX_PARAM_NAME);
  auto joints_mapping = InterfaceType::get_joints_mapping(node_,JOINTS_PREFIX_PARAM_NAME);

  for(auto & [key,value]: joints_mapping)
  {
    value= prefix+value;
  }
...
{
  auto base_frame_id = get_mobile_base_info<std::string>(node_,BASE_FRAME_ID_PARAM_NAME);
...
{
  auto odom_frame_id = get_mobile_base_info<std::string>(node_,ODOM_FRAME_ID_PARAM_NAME);
...
{
  bool enable_odom_tf = get_mobile_base_info<bool>(node_,ENABLE_ODOM_TF_PARAM_NAME);
...
void MobileBaseController<InterfaceType,KinematicType>::init_interface_()
{
  ...
  auto joints_mapping= load_joints_mapping_();
