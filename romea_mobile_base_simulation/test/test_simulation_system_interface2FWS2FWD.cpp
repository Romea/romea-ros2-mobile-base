...
protected:
  ...
  void SetUp() override
  {
    std::string xacro_file =
      std::string(TEST_DIR) + "/test_simulation_system_interface2FWS2FWD.xacro";
    ...
    interface = std::make_unique<romea::ros2::SimulationInterface2FWS2FWD>(
      romea::ros2::SimulationInterface2FWS2FWD::Configuration(info[0], "mobile_base"));
  }
...
TEST_F(TestSimulationInterface2FWS2FWD, checkGetCommandUsingJointState)
{
  ...
  EXPECT_STREQ(simulation_command.name[2].c_str(), "front_left_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[3].c_str(), "front_right_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[4].c_str(), "rear_left_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[5].c_str(), "rear_right_wheel_spinning_joint");
...
TEST_F(TestSimulationInterface2FWS2FWD, checkGetState)
{
  ...
  EXPECT_NEAR(state_interfaces[0].get_value(), command.frontLeftWheelSteeringAngle, 0.001);
  EXPECT_NEAR(state_interfaces[1].get_value(), command.frontRightWheelSteeringAngle, 0.001);
  ...
  EXPECT_NEAR(
    state_interfaces[9].get_value(), simulation_command.rearLeftWheelSpinningSetPoint, 0.001);
  EXPECT_NEAR(
    state_interfaces[12].get_value(), simulation_command.rearRightWheelSpinningSetPoint, 0.001);
}
