...
protected:
  ...
  void SetUp() override
  {
    std::string xacro_file =
      std::string(TEST_DIR) + "/test_simulation_system_interface2TTD.xacro";
    ...
    interface = std::make_unique<romea::ros2::SimulationInterface2TTD>(
      romea::ros2::SimulationInterface2TTD::Configuration(info[0], "mobile_base"));
  }
...
TEST_F(TestSimulationInterface2TTD, checkGetCommandUsingJointState)
{
  ...
  EXPECT_STREQ(simulation_command.name[2].c_str(), "left_idler_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[3].c_str(), "right_idler_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[4].c_str(), "front_left_roller_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[5].c_str(), "front_right_roller_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[6].c_str(), "rear_left_roller_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[7].c_str(), "rear_right_roller_wheel_spinning_joint");
...
TEST_F(TestSimulationInterface2TTD, checkGetState)
{
  ...
  EXPECT_NEAR(
    state_interfaces[7].get_value(), simulation_command.leftIdlerWheelSpinningSetPoint, 0.001);
  EXPECT_NEAR(
    state_interfaces[10].get_value(), simulation_command.rightIdlerWheelSpinningSetPoint, 0.001);
  EXPECT_NEAR(
