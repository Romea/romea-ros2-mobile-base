...
protected:
  ...
  void SetUp() override
  {
    std::string xacro_file =
      std::string(TEST_DIR) + "/test_simulation_system_interface2THD.xacro";
    ...
    interface = std::make_unique<romea::ros2::SimulationInterface2THD>(
      romea::ros2::SimulationInterface2THD::Configuration(info[0], "mobile_base"));
  }
...
TEST_F(TestSimulationInterface2THD, checkGetCommandUsingJointState)
{
  romea::core::HardwareCommand2TD command = {0.611111, 1.6111};
  ...
  EXPECT_STREQ(simulation_command.name[2].c_str(), "front_left_idler_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[3].c_str(), "front_right_idler_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[4].c_str(), "rear_left_idler_wheel_spinning_joint");
  EXPECT_STREQ(simulation_command.name[5].c_str(), "rear_right_idler_wheel_spinning_joint");
...
TEST_F(TestSimulationInterface2THD, checkGetState)
{
  ...
  simulation_state.rightSprocketWheelSpinningMotion.velocity =
    simulation_command.rightSprocketWheelSpinningSetPoint;
  simulation_state.frontLeftIdlerWheelSpinningMotion.velocity =
    simulation_command.frontLeftIdlerWheelSpinningSetPoint;
  simulation_state.frontRightIdlerWheelSpinningMotion.velocity =
    simulation_command.frontRightIdlerWheelSpinningSetPoint;
  simulation_state.rearLeftIdlerWheelSpinningMotion.velocity =
    simulation_command.rearLeftIdlerWheelSpinningSetPoint;
  simulation_state.rearRightIdlerWheelSpinningMotion.velocity =
...
TEST_F(TestSimulationInterface2THD, checkGetStateUsingJointState)
{
  romea::core::HardwareCommand2TD command = {0.611111, 1.6111};
