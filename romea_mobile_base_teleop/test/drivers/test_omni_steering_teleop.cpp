...
{
public:
  TestOmniSteeringTeleop() : teleop(), joy_pub(), cmd_sub() {}
  ...
  void init()
  {
    ...
    no.arguments(
      {"--ros-args", "--params-file", std::string(TEST_DIR) + "/test_omni_steering_teleop.yaml"});
  ...
  {
    joy_pub->publish(linear_speed, lateral_speed, angular_speed, slow_mode, turbo_mode);
